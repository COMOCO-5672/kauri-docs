...
| 高级工程经验 | `{序号}.player-*.md` | `2.player-av-dataflow.md` |
| 专项诊断 | `{序号}.*-diagnosis.md` 或 `{序号}.*-bsf.md` | `1.hevc-hvcc-annexb-bsf.md` |
| 风险清单 | `gaps-and-risks.md` | `gaps-and-risks.md` |
| 面试/问答 | `interview-qa.md` | `interview-qa.md` |

> [!TIP]
> 后续和播放器实现强相关的经验文档建议统一使用 `{序号}.player-` 前缀，例如 `3.player-hwdec.md`、`4.player-audio-passthrough.md`、`5.player-sync.md`。这样不会和 FFmpeg 自身模块文档混在一起。
...
> [!IMPORTANT]
> 普通 FFmpeg 软件解码路径里，播放器通常不需要自己解析 slice header，也不需要自己维护 PPS/SPS/VPS 引用关系。播放器要维护的是输入合同：`codecpar/extradata`、packet payload 格式、packet side data、decoder context 必须一致。

```mermaid
flowchart LR
    Player["Player\n维护输入合同"]
    Extra["codecpar/extradata\navcC/hvcC/AnnexB params"]
    Pkt["AVPacket\npayload + pts/dts + side data"]
    FF["FFmpeg decoder\nNAL split / parameter set / slice parse"]
    Frame["AVFrame\npicture/PCM + side data"]

    Player --> Extra --> FF
    Player --> Pkt --> FF --> Frame
```

播放器需要关心的不是“我有没有亲眼看到 VPS/SPS/PPS”，而是：

| 问题 | 播放器应该做什么 | 是否需要自己解 slice |
| --- | --- | --- |
| decoder 是否能打开 | 复制 `AVCodecParameters` 到 `AVCodecContext`，保留 extradata/coded side data | 不需要 |
| packet 能否送入 decoder | 保持 packet 格式和 extradata 合同一致 | 不需要 |
| 参数集是否可用 | 让 FFmpeg 从 extradata 或 packet 解析；必要时等待或缓存参数集 | 通常不需要 |
| seek 后缺参数集 | 丢到下一个可随机接入点，或 AnnexB 路径补缓存参数集 | 只需要识别参数集，不需要解 slice |
| 硬解 session 创建 | 某些硬件 API 需要先拿到 SPS/PPS/VPS、宽高、profile | 可能需要 parser/inspector |
| 诊断码流问题 | 打印 NAL type、packet 前缀、extradata、side data | 只做浅解析 |

> [!WARNING]
> “不需要自己管 slice”不等于“什么都不用管”。如果你把 MP4 length-prefixed packet 转成 AnnexB，却继续用原始 `hvcC` 状态打开 decoder，FFmpeg 会在 NAL split 阶段读错；这时还没进入真正的 slice 解码。

FFmpeg 内部会做这些事：

- FFmpeg `libavcodec/hevcdec.c:3155` `decode_nal_units()` 调 `ff_h2645_packet_split()`，按 `is_nalff/nal_length_size` 或 start code 拆 NAL。
- FFmpeg `libavcodec/hevcdec.c:611` HEVC slice header 读取 `pps_id`。
- FFmpeg `libavcodec/hevcdec.c:621` 根据 `pps_id` 选择 PPS。
- FFmpeg `libavcodec/hevcdec.c:625` 根据 PPS 引用的 `sps_id` 选择 SPS。
- FFmpeg `libavcodec/hevcdec.c:3304` `hevc_decode_extradata()` 解析 HEVC extradata。
- FFmpeg `libavcodec/hevcdec.c:3348` 每个 packet 前检查 `AV_PKT_DATA_NEW_EXTRADATA`。
- FFmpeg `libavcodec/h264dec.c:609` H.264 decoder 同样调用 `ff_h2645_packet_split()`。
- FFmpeg `libavcodec/h264_slice.c:1718` H.264 slice header 读取 `pps_id`。
- FFmpeg `libavcodec/h264_slice.c:1723` 如果对应 PPS 不存在，会报参数集缺失。

#### 什么时候可以打开 decoder

不要把“没看到 VPS/SPS/PPS”直接等同于“不能打开 decoder”。

| 情况 | 软件 FFmpeg decoder | 自研/平台硬解 |
| --- | --- | --- |
| MP4 有 `avcC/hvcC` extradata | 可以打开 | 通常可以打开 |
| `hvcC` 很短、数组为空，但 packet 内可能带参数集 | 可以打开并继续送 packet | 先确认硬件 API 是否允许 |
| TS/M2TS AnnexB，参数集可能在后续 packet | 可以打开，先送，等待 decoder 建立参数集 | 通常先 parser 到参数集再建 session |
| raw stream 无 extradata，尚未 parser 到参数集 | 可尝试 parser/decoder，但起播信息不足 | 不建议创建硬件 session |

> [!TIP]
> 软件解码优先把正确的 demux 输出交给 FFmpeg；硬解和自研低层接口才更需要在创建 session 前主动提取参数集、宽高、profile、bit depth 和 surface 格式。

#### 什么时候需要更新参数集

播放器不需要解 slice，但需要识别“参数合同发生变化”的信号：

| 信号 | 说明 | 工程动作 |
| --- | --- | --- |
| packet 内出现新的 VPS/SPS/PPS | AnnexB 或样本内参数集更新 | 更新缓存；必要时重建硬解 session |
| `AV_PKT_DATA_NEW_EXTRADATA` | 容器通知 codec private data 变化 | 保留 side data；确认 decoder 支持或重建 |
| 输出 frame 分辨率/格式变化 | decoder 已解析到新参数 | 重建 filter、renderer、surface pool |
| 硬解返回 profile/level 不支持 | 硬件能力不匹配 | fallback 软件解码或禁用该硬解 |
| seek 后连续缺参数集 | 随机接入点不完整 | 向前 seek、等下个 IDR/IRAP、AnnexB 路径补缓存 |

判断逻辑可以写成：

```text
open decoder:
    如果 codec_id 已知，且 codecpar/extradata 合同清楚，软件解码可以打开
    如果硬解需要参数集，先 parser/inspect 到 SPS/PPS/VPS 再创建 session

send packet:
    保留 packet side data
    确认 packet 格式和 decoder extradata 状态一致
    正常送给 FFmpeg

on decoder error:
    如果是 missing PPS/SPS/VPS:
        不要立刻判文件坏
        检查 extradata、packet NAL type、seek 状态、NEW_EXTRADATA
        丢到下一个可随机接入点或重建 decoder
```

> [!IMPORTANT]
> 最准确的一句话：播放器不要重复实现 codec decoder；播放器要维护 demux、packet、decoder、输出链路之间的数据合同。

### decoder 是怎么知道 hvcC、avcC、AnnexB 的

这里要分清两个动作：`avcodec_parameters_to_context()` 负责把流级合同复制到 `AVCodecContext`，真正“理解 extradata 是 hvcC/avcC 还是 AnnexB 参数集”的动作发生在 decoder open 和 packet split 过程中。

```mermaid
sequenceDiagram
    participant Demux as Demuxer
    participant Par as AVCodecParameters
    participant Ctx as AVCodecContext
    participant Dec as Decoder

    Demux-->>Par: codec_id + codec_tag + extradata
    Par-->>Ctx: avcodec_parameters_to_context()
    Note over Ctx: 这里只是保存 extradata 字节和基础参数
    Ctx-->>Dec: avcodec_open2()
    Note over Dec: 解析 avcC/hvcC/AnnexB 参数集\n设置 nal_length_size / is_nalff 等内部状态
    Demux-->>Dec: AVPacket payload
    Note over Dec: 按内部状态拆 NAL\n再解析 VPS/SPS/PPS/slice
```

| 阶段 | 做了什么 | 对播放器的含义 |
| --- | --- | --- |
| demux | 从 MP4/MKV/TS 读出 codec private data，放入 `AVCodecParameters.extradata` 或 side data | 不要丢 `codecpar`，不要自己简化字段 |
| `avcodec_parameters_to_context()` | 把 `codec_id`、宽高、音频参数、`extradata` 等复制到 `AVCodecContext` | 它不是完整解析器，只是建立 decoder 输入合同 |
| `avcodec_open2()` | decoder 读取 extradata，识别 `avcC/hvcC`、NAL length size、参数集 | 这里才开始建立 codec 内部状态 |
| `avcodec_send_packet()` | decoder 按内部状态拆 packet；MP4 NALFF 用长度字段，AnnexB 用 start code | packet 格式必须和 decoder 状态一致 |
| packet side data | `AV_PKT_DATA_NEW_EXTRADATA` 可以在中途带新 extradata | 播放器封装 packet 时必须保留 side data |

> [!IMPORTANT]
> 所以答案不是“在 `avcodec_parameters_to_context()` 的时候 decoder 就全知道了”。更准确地说：`avcodec_parameters_to_context()` 把 `hvcC/avcC` 这类字节交给 decoder；decoder 在 `avcodec_open2()` 和后续送包时解析这些字节并决定如何拆 NAL。

对 HEVC 来说：

- MP4 `hvcC` 通常表示后续 packet 是 length-prefixed NALFF，decoder 要按 NAL length size 读取每个 NAL。
- TS/M2TS 或 raw AnnexB 通常没有 `hvcC`，packet 里直接有 `00 00 01` / `00 00 00 01` start code。
- 如果你插入 `hevc_mp4toannexb`，packet 形态从 NALFF 变成 AnnexB；下游 decoder context、extradata 或 BSF 输出参数必须同步。
- `hvcC` 很短不等于脏数据。某些 MP4 的参数集可能在样本内，或者 `hvcC` 只提供最小配置；要结合 packet 前缀和 NAL type 判断。

> [!WARNING]
> 典型错误是：decoder 根据 `hvcC` 认为 packet 是 NALFF，但你已经把 packet 改成 AnnexB；或者 decoder 认为 packet 是 AnnexB，但实际喂的是 MP4 length-prefixed。前者常见现象就是 `Invalid NAL unit size`，因为 decoder 把 start code 或 NAL header 当成长度字段读了。

播放器需要自己补的不是完整 slice 解码，而是一个轻量 bitstream inspector：

| 检查项 | 怎么看 | 目的 |
| --- | --- | --- |
| extradata 类型 | 前几个字节、容器 sample entry、`extradata_size` | 判断 decoder 初始化合同 |
| packet 前缀 | `00 00 00 xx` 更像 NALFF；`00 00 01` 更像 AnnexB | 判断 packet payload 合同 |
| NAL type 列表 | 只扫 NAL header，不解 slice header | 看是否有 VPS/SPS/PPS、IDR/CRA、DOVI RPU |
| side data | 枚举 `AVPacket.side_data` | 不漏 `NEW_EXTRADATA`、DOVI、HDR |
| decoder 状态 | 打印 open 时 extradata 和实际送包格式 | 定位 BSF/缓存/seek 后错配 |

相关源码：

- FFmpeg `libavcodec/codec_par.c:226` `avcodec_parameters_to_context()` 复制 `AVCodecParameters` 到 `AVCodecContext`。
- FFmpeg `libavcodec/hevcdec.c:3304` `hevc_decode_extradata()` 解析 HEVC extradata。
- FFmpeg `libavcodec/hevcdec.c:3155` `decode_nal_units()` 根据内部 bitstream 状态拆 NAL。
- FFmpeg `libavcodec/hevcdec.c:3348` 处理 `AV_PKT_DATA_NEW_EXTRADATA`。
- FFmpeg `libavcodec/h264dec.c:609` H.264 decoder 进入同类 NAL split 流程。

### FFmpeg 已经做完什么，播放器还要补什么

这张图回答“哪些解析交给 FFmpeg，哪些仍然是播放器责任”。软件解码和硬解都要看这条边界；硬解只是对输入参数和输出 surface 更严格。

```mermaid
flowchart TB
    Demux["libavformat demuxer\ncontainer parse"]
    Par["AVCodecParameters\nextradata + coded_side_data"]
    Ctx["AVCodecContext\navcodec_parameters_to_context"]
    Pkt["AVPacket\npayload + side_data"]
    Dec["libavcodec decoder\nNAL/OBU/parser/slice"]
    Frame["AVFrame\npixels/PCM + frame side_data"]
    Player["Player\nseek/fallback/render/AO/sync"]

    Demux --> Par --> Ctx --> Dec
    Demux --> Pkt --> Dec --> Frame --> Player
    Player -. "必须保留/匹配" .-> Par
    Player -. "必须保留/匹配" .-> Pkt
```
...
| 能力 | FFmpeg 负责的部分 | 播放器不应重复实现 |
| --- | --- | --- |
| 容器配置解析 | MP4 `avcC/hvcC/esds/dvcC`、TS descriptor、MKV CodecPrivate | 不要自己重写完整容器解析 |
| `AVCodecParameters` 到 decoder | `avcodec_parameters_to_context()` 复制 codec id、extradata、宽高、音频参数 | 不要手动漏字段拷贝 |
| 格式识别 | H.264/HEVC decoder 解析 extradata，设置 `is_avc/is_nalff` 和 `nal_length_size` | 不要每包猜格式 |
| NAL 拆分 | `ff_h2645_packet_split()` 按 NALFF 或 AnnexB 拆包 | 不要重复写完整 NAL splitter |
| 参数集和 slice | decoder 解析 VPS/SPS/PPS、slice header、引用关系 | 不要自己解 slice |
| packet side data 部分传递 | 常见 HDR、skip samples、DOVI config 等由 decoder 或通用层处理 | 不要丢 side data |
| frame 属性补齐 | `AVFrame` 的 PTS、duration、色彩、音频参数等 | 不要绕过 `avcodec_receive_frame()` |

播放器仍然必须补齐：

| 播放器责任 | 如果没做好会怎样 | 硬解时是否更严格 |
| --- | --- | --- |
| 保留完整 `AVCodecParameters` | decoder 初始认知错误，extradata 丢失 | 是 |
| 保留 packet side data | `NEW_EXTRADATA`、DOVI、HDR、skip samples 丢失 | 是 |
| 保证 packet 格式和 decoder 状态一致 | MP4 NALFF/AnnexB 错配，出现 `Invalid NAL unit size` | 是 |
| seek/flush 后从合理位置喂包 | 缺参数集、缺参考帧、黑屏到下个 IDR | 是 |
| BSF 输出参数同步 | BSF 后 packet 改了，但 decoder 仍按旧格式拆 | 是 |
| 硬件能力选择和 fallback | 硬解打开成功但 receive 失败、黑屏、花屏 | 是，核心责任 |
| 渲染/AO 能力适配 | HDR/DV 元数据没消费、GPU surface 不支持、音频透传失败 | 是 |
| A/V sync | PTS 有了但播放节奏错 | 一样严格 |

> [!IMPORTANT]
> 做硬解时不要把“FFmpeg 已经会解析参数集”理解成“播放器不需要关心参数”。软件 decoder 可以边读边建立内部状态；很多硬件 API 创建 session 前就需要宽高、profile、bit depth、SPS/PPS/VPS 或对应 codec private data。

硬解场景下，播放器至少要在创建或选择硬解前确认：

- codec：H.264、HEVC、AV1、VP9 等；
- profile/level/tier；
- bit depth 和 chroma format；
- coded size / visible size；
- extradata 类型：`avcC/hvcC/av1C` 还是 AnnexB；
- packet 是否经过 BSF；
- 是否有 `AV_PKT_DATA_NEW_EXTRADATA`；
- 目标硬件 API 是否接受当前 bitstream 合同；
- 输出 surface 格式是否能被 renderer 接住；
- 失败时是否能回退软件解码并重送关键 packet。

相关源码：

- FFmpeg `libavcodec/avcodec.h:2348` `avcodec_parameters_to_context()` 是从 `AVCodecParameters` 建立 decoder context 的常规入口。
- FFmpeg `libavcodec/avcodec.h:1962` `hw_device_ctx` 描述硬件设备。
- FFmpeg `libavcodec/avcodec.h:1919` `hw_frames_ctx` 描述硬件帧池。
- FFmpeg `libavcodec/decode.c:1055` 根据 `hw_device_ctx` 和 codec hw config 选择硬件输出格式。
- FFmpeg `libavcodec/decode.c:1109` `ff_decode_get_hw_frames_ctx()` 创建/取得硬件 frames context。
- FFmpeg `libavcodec/decode.c:1218` `hwaccel_init()` 初始化 hwaccel。
- mpv `video/decode/vd_lavc.c:495` 选择硬解方法。
- mpv `video/decode/vd_lavc.c:750` 设置 hwaccel flags。
- mpv `video/decode/vd_lavc.c:767` 设置 `AVCodecContext.hw_device_ctx`。
- mpv `video/decode/vd_lavc.c:927` 初始化通用 hw frames context。
- mpv `video/decode/vd_lavc.c:1288` 硬解失败后 fallback 并重送缓存 packet。
...
如果插 `hevc_mp4toannexb`，就必须让下游 decoder 看到 AnnexB-compatible 的参数，而不能继续复用原始 `hvcC` 初始化状态。细节见 [HEVC hvcC / AnnexB / BSF 诊断](1.hevc-hvcc-annexb-bsf.md)。
...
sequenceDiagram
    ...
    participant Avctx as AVCodecContext
    participant HW as HW device/frames
    participant Dec as Decoder
    participant VO as Renderer

    Demux-->>Player: codecpar + extradata + side data
    Player->>Player: inspect codec/profile/bit depth/packet format
    Player->>HW: create/check device
    Player->>Avctx: avcodec_parameters_to_context()
    Player->>Avctx: set hw_device_ctx/get_format/hw_frames_ctx
    Player->>Dec: avcodec_open2()
    loop packets
        Demux-->>Player: AVPacket + side data
        Player->>Dec: avcodec_send_packet()
        Dec-->>Player: AVFrame hw surface
        Player->>VO: zero-copy render or copy-back
    end
```

硬解实现建议按这个顺序落地：

1. **先用 demuxer 建立流级合同**：拿到 `codecpar`、extradata、coded side data、timebase。
2. **做硬件能力预判**：codec、profile、level、bit depth、chroma、分辨率、DV/HDR、是否 10bit。
3. **确认 bitstream 合同**：MP4 NALFF、AnnexB、是否已过 BSF、extradata 是否和 packet 匹配。
4. **创建硬件设备**：D3D11VA/DXVA2/CUDA/VAAPI/QSV/VideoToolbox/MediaCodec 等。
5. **设置 `AVCodecContext`**：`avcodec_parameters_to_context()` 后设置 `hw_device_ctx`、`get_format`、必要的 flags。
6. **打开 decoder**：`avcodec_open2()`。
7. **接收硬件 frame**：确认 `AVFrame.format` 和 `hw_frames_ctx`。
8. **连接渲染链路**：能 zero-copy 就直接渲染，否则 copy-back。
9. **fallback**：硬解失败时重建软件 decoder，并重送关键 packet。

> [!WARNING]
> 硬解失败不能只看 `avcodec_open2()`。有些 hwaccel 打开成功，但在 `avcodec_receive_frame()` 才失败；也可能 decoder 成功输出 GPU surface，但 renderer 不支持这个 surface，最终表现为黑屏。

### 硬解和 BSF 的关系

硬解最容易把 BSF 问题放大。原因是部分硬件 API 或 wrapper 只接受某种 bitstream 形态。

| 输入来源 | 可能的 packet 形态 | 硬解注意点 |
| --- | --- | --- |
| MP4 H.264/HEVC | `avcC/hvcC + length-prefixed packet` | FFmpeg hwaccel 通常可由 decoder 处理；自研硬件接口未必接受 |
| TS/M2TS H.264/HEVC | AnnexB start code packet | 通常适合硬件低层接口，但需要参数集随机接入 |
| BSF 后 MP4 | AnnexB packet + BSF output params | 必须用 BSF 输出合同初始化下游 |
| raw AnnexB | AnnexB packet，无容器 timebase | 需要 parser 推断参数和帧边界 |

> [!IMPORTANT]
> 如果你走 FFmpeg hwaccel，优先保持 FFmpeg demuxer/decoder 的自然合同，不要过早手动插 BSF。只有目标硬件接口明确要求 AnnexB，或者你绕过 FFmpeg decoder 直连平台 API，才考虑主动转换。

### FFmpeg hwaccel 和自研平台硬解不是同一层

很多硬解问题来自“以为自己在用 FFmpeg 解码，实际已经绕开了 FFmpeg 的 codec 解析能力”。要先判断你的链路属于哪一种。

| 链路 | packet 送给谁 | 谁解析 `hvcC/avcC` 和参数集 | 谁决定硬件 surface | 播放器重点 |
| --- | --- | --- | --- | --- |
| FFmpeg 软件解码 | `avcodec_send_packet()` | FFmpeg decoder | 不涉及 GPU surface | 保持 `codecpar + packet + side data` 一致 |
| FFmpeg hwaccel | `avcodec_send_packet()` | FFmpeg decoder + hwaccel | FFmpeg `hw_device_ctx/hw_frames_ctx` 与 renderer | 保持自然合同，做好硬件选择和 fallback |
| FFmpeg 解码后 copy-back | `avcodec_send_packet()` | FFmpeg decoder + hwaccel | FFmpeg 输出 GPU frame，再 copy 到 CPU | 性能较低但兼容性好 |
| 自研直连 D3D/MediaCodec/VT | 平台 API input buffer | 你或平台 wrapper | 你创建/管理 surface | 必须明确 API 要 NALFF 还是 AnnexB，必须自己准备 config |
| BSF 后直连硬件 API | 平台 API input buffer | BSF 转 payload，你负责同步 params | 你创建/管理 surface | BSF 输出合同必须成为硬件初始化合同 |

> [!IMPORTANT]
> 如果你用的是 FFmpeg hwaccel，仍然应该让 FFmpeg decoder 看到原始 demux 合同：MP4 就保留 `hvcC/avcC + NALFF packet`，TS/M2TS 就保留 AnnexB packet。不要为了“硬解”先把所有东西都转 AnnexB，除非你确认目标 API 的输入合同就是 AnnexB。

自研硬解路径要多做一层“硬件输入合同”：

```mermaid
flowchart TB
    Par["AVCodecParameters\nextradata/coded side data"]
    Inspect["Bitstream inspector\nNAL type + packet prefix"]
    Decide["Input contract decision\nNALFF or AnnexB"]
    Config["Hardware config\nSPS/PPS/VPS or codec private data"]
    Queue["Hardware input queue\nencoded packets"]
    Surface["Hardware surface\nNV12/P010/D3D11/VT/MediaCodec"]

    Par --> Inspect --> Decide
    Decide --> Config --> Queue --> Surface
    Inspect -. "发现 NEW_EXTRADATA/参数变化" .-> Config
```
...
| 问题 | 工程判断 |
| --- | --- |
| API 接受 MP4 NALFF 还是 AnnexB | 看平台文档和本地验证；不要按 codec 名字猜 |
| 初始化 config 需要什么 | H.264 常见 SPS/PPS；HEVC 常见 VPS/SPS/PPS 或 `hvcC`；AV1 常见 sequence header |
| 参数集从哪里来 | 优先 extradata；没有则从 packet 参数集 NAL 提取；中途看 `NEW_EXTRADATA` |
| 每个关键帧前是否要补参数集 | AnnexB 低层 API 常见需要；FFmpeg decoder 通常不需要播放器补 |
| seek 后怎么恢复 | flush 硬件队列，丢到可随机接入点，必要时先送缓存参数集 |
| 参数变化怎么办 | 重建 session/surface pool，重送新参数后的关键 packet |

> [!WARNING]
> 硬解里最危险的不是“没解析 slice”，而是“初始化 config、packet payload、surface 格式三者不是同一套合同”。比如 config 来自 `hvcC`，packet 却已经被转成 AnnexB；或者 surface 是 8bit NV12，但 HEVC Main10 实际需要 P010。

实践上可以把硬解入口写成一个决策树：

```text
if use_ffmpeg_hwaccel:
    copy codecpar -> avctx
    set hw_device_ctx/get_format
    keep demux packet format unchanged unless FFmpeg path explicitly requires BSF
    send AVPacket with side data
else if use_platform_decoder_directly:
    inspect extradata and first packets
    decide required input format: NALFF or AnnexB
    convert only once at the boundary
    build hardware config from extradata or extracted VPS/SPS/PPS
    on NEW_EXTRADATA or SPS/PPS/VPS change, recreate session if needed
```
...
| 变化 | 软件解码 | 硬解处理 |
| --- | --- | --- |
| `AV_PKT_DATA_NEW_EXTRADATA` | decoder 可能内部更新 | 通常要评估是否重建 session |
| 分辨率变化 | 可继续输出新 frame | 重建 surface pool / renderer target |
| bit depth/pixel format 变化 | decoder 更新 output format | 重建硬件 frames context |
| profile/level 超硬件能力 | 软件仍可能解 | fallback 软件 |
| DV/HDR metadata 变化 | frame side data 更新 | renderer/tone map 状态更新 |

硬解相关日志必须包含：

```text
[hwdec] request=d3d11va codec=hevc profile=Main10 level=...
[hwdec] extradata_type=hvcC extradata_size=23 packet_format=nalff
[hwdec] device=created frames_ctx=created hw_pix_fmt=d3d11 sw_format=p010
[hwdec] receive frame fmt=d3d11 3840x1608 side_data=DOVI_RPU
[hwdec] fallback reason=unsupported profile / receive error / renderer unsupported
```

> [!TIP]
> 你现在做硬解，建议把“decoder open 前参数”和“第一帧 receive 后参数”都打出来。硬解很多问题只有在第一帧输出后才能确认：真正输出的 `AVFrame.format`、`sw_format`、surface 尺寸、bit depth、色彩 metadata 是否和预期一致。
